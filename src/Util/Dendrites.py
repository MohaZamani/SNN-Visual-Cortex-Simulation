...
class SparseDendriticInput(BaseDendriticInput):
    ...
    def initialize(self, synapse):
        ...
        if not synapse.network.transposed_synapse_matrix_mode:
            raise RuntimeError(
                "Network should've made with SxD mode for synapses")
...
class SimpleDendriticInput(BaseDendriticInput):
    ...
    def initialize(self, synapse):
        ...
        if not synapse.network.transposed_synapse_matrix_mode:
            raise RuntimeError(
                f"Network should've made with SxD mode for synapses")
...
class LateralDendriticInput(BaseDendriticInput):
    ...
    def initialize(self, synapse):
        ...
        self.padding = tuple(
            ((synapse.W.shape[i] - 1) // 2) for i in range(0, 2))
        ...
        if not synapse.W.numel() % 2:
            raise RuntimeError(
                f"Synapse {synapse.src.tags[0]}=>{synapse.dst.tags[0]}: For lateral connection weight should not have any even size dimension. {synapse.W.shape}"
            )
    ...
    def calculate_input(self, synapse):
        ...
        I = F.conv3d(input=spikes, weight=synapse.W,
...
class Conv2dDendriticInput(BaseDendriticInput):
    ...
    def initialize(self, synapse):
        ...
        if not (
            ...
            <= (
                (
                    (synapse.src.height + 2 *
                     padding[0] - synapse.weights.size(2))
                    / stride[0]
                )
                + 1
            )
        ...
        if not (
            ...
            <= (
                (
                    (synapse.src.width + 2 *
                     padding[1] - synapse.weights.size(3))
                    / stride[1]
                )
                + 1
            )
        ...
        if (
            not (
                ...
                <= (
                    (
                        (synapse.src.height + 2 *
                         padding[0] - synapse.kernel_shape[4])
                        / stride[0]
                    )
                    + 1
                )
        ...
        if (
            not (
                ...
                <= (
                    (
                        (synapse.src.width + 2 *
                         padding[1] - synapse.kernel_shape[5])
                        / stride[1]
                    )
                    + 1
                )
